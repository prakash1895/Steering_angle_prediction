...
#SBATCH -n 4
#SBATCH -p gpu
#SBATCH -o DL_project_commai.out
#SBATCH -t 48:00:00
#SBATCH --gres=gpu:2
...
fh = open('python_output_commai.txt', 'a')
...
def commai_model():
    model = Sequential()
    model.add(Conv2D(16, 8, 8, input_shape = input_shape, subsample=(4, 4), border_mode="same", activation='relu'))
...
with open('commai_model.yaml', 'w') as yaml_file:
    yaml_file.write(model_yaml)
print ('model.yaml saved to disk')

filepath="weights_commai.hdf5"
...
np.savetxt('history_commai.csv', history_data, delimiter=',')
