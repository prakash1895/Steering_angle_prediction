...
#SBATCH -n 4
#SBATCH -p gpu
#SBATCH -o DL_project_nvidia.out
#SBATCH -t 48:00:00
#SBATCH --gres=gpu:2
...
from keras.layers import Dropout, ELU, Lambda
...
fh = open('python_output_nvidia.txt', 'a')
...
def nvidia_model():
    model = Sequential()
    model.add(Conv2D (24, 5, 5, input_shape=input_shape, subsample=(2,2), border_mode='same', activation='relu'))
...
with open('nvidia_model.yaml', 'w') as yaml_file:
    yaml_file.write(model_yaml)
print ('model.yaml saved to disk')

filepath="weights_best_nvidia.hdf5"
...
np.savetxt('history_nvidia.csv', history_data, delimiter=',')
